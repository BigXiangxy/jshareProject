...
public class TagAliasOperatorHelper {
    ...
    public Map<String, Object> onTagOperatorResult(Context context, JPushMessage jPushMessage) {
        ...
        map.put("CheckTag", jPushMessage.getCheckTag());
        map.put("ErrorCode", jPushMessage.getErrorCode());//errorCode == 6002 || errorCode == 6014会重试
        map.put("MobileNumber", jPushMessage.getMobileNumber());
        map.put("Sequence", jPushMessage.getSequence());
        map.put("TagCheckStateResult", jPushMessage.getTagCheckStateResult());
        map.put("Tags", jPushMessage.getTags());
        init(context);
        ...
        if (jPushMessage.getErrorCode() == 0) {//成功
            ...
            Logger.e(TAG, logs);
            if (!RetryActionIfNeeded(jPushMessage.getErrorCode(), tagAliasBean)) {//不重试就显示个toast提示
                ExampleUtil.showToast(logs, context);
            }
        }
        return map;
    ...
    public Map<String, Object> onCheckTagOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
    ...
    public Map<String, Object> onAliasOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
    ...
    //设置手机号码回调
    public Map<String, Object> onMobileNumberOperatorResult(Context context, JPushMessage jPushMessage) {
        int sequence = jPushMessage.getSequence();
        ...
        if (jPushMessage.getErrorCode() == 0) {
            ...
            Logger.e(TAG, logs);
            if (!RetrySetMObileNumberActionIfNeeded(jPushMessage.getErrorCode(), jPushMessage.getMobileNumber())) {//不重试就显示个toast提示
                ExampleUtil.showToast(logs, context);
            }
        }
        return map;
